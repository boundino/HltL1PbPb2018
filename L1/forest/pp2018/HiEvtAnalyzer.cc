...
HiEvtAnalyzer::HiEvtAnalyzer(const edm::ParameterSet& iConfig) :
  ...
  TowerTag_(consumes<CaloTowerCollection>(iConfig.getParameter<edm::InputTag>("TowerSrc"))),
...
HiEvtAnalyzer::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  if (doCentrality_) {
    ...
    iEvent.getByToken(TowerTag_, ts);
    ...
    for (const auto& cal : *towers) {
      ...
      towerE.push_back(cal.energy());
      towerieta.push_back(cal.ieta());
      if(cal.energy()>towerEmax) towerEmax = cal.energy();
      ntower++;
    }
  }
...
HiEvtAnalyzer::beginJob()
{
  ...
  thi_->Branch("ntower",&ntower,"ntower/I");
  thi_->Branch("towerEmax",&towerEmax,"towerEmax/F");
  thi_->Branch("towerE",&towerE);
  thi_->Branch("towerieta",&towerieta);
