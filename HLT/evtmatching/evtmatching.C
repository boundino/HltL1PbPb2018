...
int evtmatching(TString infforest, TString infhlt, TString outfile, Long64_t nentries = -1)
{
  ...
  TTree* ntDkpi = (TTree*)fforest->Get("Dfinder/ntDkpi");
  TTree* ntDPhikkpi = (TTree*)fforest->Get("Dfinder/ntDPhikkpi");
  TTree* hlt = (TTree*)fhlt->Get("hltbitanalysis/HltTree");

  TFile* outf = new TFile(outfile,"recreate");
  TDirectory* dhiEvtAnalyzer = outf->mkdir("hiEvtAnalyzer", "");
  TDirectory* dDfinder = outf->mkdir("Dfinder", "");
  TDirectory* dhltbitanalysis = outf->mkdir("hltbitanalysis", "");

  dhiEvtAnalyzer->cd();
  TTree* new_hiroot = hiroot->CloneTree(0);
  dDfinder->cd();
  TTree* new_ntDkpi = ntDkpi->CloneTree(0);
  TTree* new_ntDPhikkpi = ntDPhikkpi->CloneTree(0);
  dhltbitanalysis->cd();
  ...
  std::cout<<hiroot->GetEntries()<<", "<<ntDkpi->GetEntries()<<", "<<ntDPhikkpi->GetEntries()<<", "<<hlt->GetEntries()<<std::endl;
